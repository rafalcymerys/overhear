...
- **Dictation mode**: Records in batches — accumulates chunks until 1.5s of silence, then transcribes that batch and keeps listening. Batches are capped at 30s. A batch shorter than 0.3s, or one in which the level never crossed the silence threshold, is discarded without transcribing. Activated/deactivated via commands from the Swift host. Stopping is a hard stop: an utterance still being recorded is discarded, and a transcription still running when the stop arrives is dropped rather than pasted — nothing reaches the user's document after they press Stop.
