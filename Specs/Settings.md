...
**Listening Mode** group, and **Strip transcription annotations**, **Ensure
spaces around inserted text** and **Match the case of the current sentence**
under a **Transcription** heading, in that order.
...
Assert: on a fresh install all five toggles are on.
...
Assert: **Match the case of the current sentence** carries the explanation its
own spec gives it, and what it does is specified in `Specs/Casing.md`.
...
1. Change every setting from its default: turn the five General toggles to
