...
| `InsertionCasing.swift` | Whether the front of the insertion is cased to suit the sentence it joins |
| `CaretContext.swift` | What the two insertion rules are given: the text before the caret and the character after it |
...
**Loading is asked for, not assumed.** `WhisperTranscriber.configuration(for:)` sets `load: true` explicitly. WhisperKit's initialiser decides for itself with `config.load ?? (config.modelFolder != nil)`, so a configuration that stays quiet about loading and names no model folder means *do not load*: the initialiser returns having only located the weights, and every CoreML model is then loaded lazily by whichever call needs one first. That call is the user's first transcription — plus a tokenizer download on a first run — which is exactly the moment the engine's start-at-launch arrangement exists to avoid. `isLoaded` is the distinction made visible: weights in memory, not merely files found.
...
The one thing that has to come from outside is those two characters, and that is where the Accessibility API does get used after all — for reading, never for writing. `FocusedField` asks the focused element for its selected range and then for a short run of text before it and the single character after it, preferring `kAXStringForRangeParameterizedAttribute` over copying the whole document out of the application on every utterance, and falling back to `kAXValue` for elements that will not serve a range. Every part of it is best effort: a password field, a terminal or an editor that draws its own text yields `CaretContext.unknown`, and the transcription goes in exactly as it did before any of this existed. Messaging is capped at 250ms so an unresponsive application costs a missing space rather than a visible stall.

**Casing.** The same problem one layer up: a transcription arrives as a sentence, capital at the front, and that capital is wrong the moment the insertion lands halfway through something already being written. `InsertionCasing` asks two questions that know nothing about each other — which character to change, which is about the insertion, and which way to change it, which is about the field. `Specs/Casing.md` is the rule. The first steps over leading whitespace and anything that opens, then applies only if what it reaches is a letter, so `1999 was a good year` does not come back as `1999 Was a good year`. The second reads backwards from the caret, past spaces and past brackets on either side, because what decides is rarely the character the insertion touches: a sentence that ended inside quotation marks has a bracket or two and a space between its full stop and the caret. Running out of text to read is an answer rather than a failure — nothing before the caret is the start of the field — which is also what makes an unreadable field fall back to a capital rather than to a guess. A word that spells itself keeps its case: one test for a capital anywhere but the front covers `NASA`, `iPhone` and `McDonald` at once, and the English `I` is asked for separately, having no second letter to give it away.

Casing runs before spacing and both run off one read of the field. Two reads would cost twice as much for the same two answers, and a caret that moved between them would have the rules disagree about where the insertion landed. Which is why `CaretContext` carries a run of text before the caret and a single character after it: spacing wants the one character meeting the insertion, casing wants the sentence, and nothing wants more than a character on the way out.
...
| Match the case of the current sentence | `matchSentenceCase` | on | Whether the front of an insertion is cased to suit the sentence it joins |
...
- `InsertionCasingTests` — `Specs/Casing.md` with the sentence before the caret written out by hand: which way each boundary decides, the words that keep their case against the rule, and the insertions with no case to change. `TextInjectorTests` covers that the cased string is what reaches the pasteboard, that casing runs before spacing, and that one read of the field serves both.
